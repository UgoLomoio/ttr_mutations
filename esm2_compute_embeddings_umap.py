...
#df_consequences['Consequence'] = df_consequences['Consequence'].replace({'Likely pathogenic': 'Unknown'})
...
if __name__ == "__main__":
    ...
    for npz_file in npz_files:
        if "WT" in npz_file.upper():
            ...
            label = "<b>" + label + "</b>"  # Make the label bold for HTML rendering
    ...
    color_map = {
        "<b>Pathogenic</b>": "red",
        "<b>Likely pathogenic</b>": "orange",
        "<b>Unknown</b>": "black",
        "<b>Benign</b>": "green"
    }

    df = pd.DataFrame({"<b>UMAP-1</b>": emb2d[:,0], "<b>UMAP-2</b>": emb2d[:,1], "Consequence": labels, "Variant": variants})
    fig = px.scatter(df, x='<b>UMAP-1</b>', y='<b>UMAP-2</b>', color='Consequence', hover_data='Variant', color_discrete_map=color_map)
    ...
    fig.update_layout(legend = dict(font = dict(family = "Courier", size = 40, color = "black")),
                  legend_title = dict(text = "<b>Clinical Significance</b>", font = dict(family = "Courier", size = 48, color = "blue")))

    fig.update_layout(title=dict(text='<b> ESM2+UMAP 2D representation of TTR variants</b>', font = dict(family = "Courier", size = 68, color = "black")),
    ...
    pio.write_image(fig, results_path + sep + "umap_plot.png", format="png", scale=4)  # scale increases resolution
    fig.show()
