...
sdf_types = ["existing", "optimized", "generated"] #existing, optimized or generated

if "existing" in sdf_types:
    dockers =  ["AutoDock"]#["DiffDock", "AutoDock"] #DiffDock or AutoDock
else:
...
for sdf_type in sdf_types:
    ...
    if "optimized" in sdf_types:
        if sdf_type == "existing":
            ligands = ["tafamidis", "acoramidis", "tolcapone", "diflunisal"]
            ligands_dict[sdf_type] = ligands
        else:
            ligands = [ligand.split(".")[0] for ligand in os.listdir(ligand_dir) if ".sdf" in ligand]
            ligands = [ligand for ligand in ligands if ligand.endswith("_preprocessed")]
            ligands_dict[sdf_type] = ligands 
    else:
        ligands = [ligand.split(".")[0] for ligand in os.listdir(ligand_dir) if ".sdf" in ligand]
        ligands = [ligand for ligand in ligands if ligand.endswith("_preprocessed")]
...
if __name__ == "__main__":
    ...
    for docker in dockers:
            ...
            for ligand in ligands:
                ...
                if sdf_type == "generated": 
                    ...
                    if reference_ligand in dfs["existing"][docker]:
                        ...
                        if "Mutation" in df_acora.columns:
                ...
                if sdf_type == "optimized":
                    ...
                    #print(diff_opt)
                    plot_difference(diff_opt, mode = "optimized", title= f"{docker}: Optimised - Original Tafamidis", filename = f"diffwt_{d}_optimized.pdf")
                    ...
                    if ligand == "acoramidis":
                        ...
                        df_acora = dfs[sdf_type][docker]['acoramidis']
                        df_ligand = dfs[sdf_type][docker][ligand]
                        if "Mutation" in df_ligand.columns:
                            df_ligand.set_index('Mutation', inplace=True)
                        if "Mutation" in df_acora.columns:
                            df_acora.set_index('Mutation', inplace=True)
                        diff_gen = df_acora[score_label] - df_ligand[score_label]
                        plot_difference(diff_gen, mode = "optimized", title= f"{docker}: {ligand} - Acoramidis", filename = f"diffwt_{d}_{ligand}_acora.pdf")
        ...
        if sdf_type in ["existing", "optimized"] and "AutoDock" in dockers:
            # Find the best Vina score for each ligand

            best_tafamidis_score = dfs["existing"]["AutoDock"]['tafamidis']['Vina Score'].mean()
            best_acoramidis_score = dfs["existing"]["AutoDock"]['acoramidis']['Vina Score'].mean()